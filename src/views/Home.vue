  ...
  <div>
    ...
    <div class="home" ref="wrapper">
      ...
      <div v-else-if="!lock && value.trim()">
        <div v-for="(item, index) in list" :key="index" class="goods" @click="goSearch(item)">
          <div class="img">
      ...
      <div v-else>
        <div v-if="history.length">
          <div class="searchTitle">
            ...
            <div class="icon" @click="removeHistories"><van-icon name="delete" /></div>
          </div>
          <div class="allHistory">
            <div v-for="(item,index) in history" :key="index" class="history">
              <div class="icon" @click="removeHistory(index)"><van-icon name="cross"/></div>
              <div><van-tag plain round size="large" @click="goHistory(item)">{{item}}</van-tag></div>
            </div>
          </div>
        </div>
  ...
  methods: {
      ...
      try {
        ...
        if (res.code === 200) {
          ...
          this.list.map(item => {
            ...
            item.name = findText.join('<span style="color:red;">' + val + '</span>'); //在拆分的数组后加入span并转化成字符串
          });
          this.$nextTick(() => {
            this.scroll = new BScroll(this.$refs.wrapper, {
              scrollY: true,
    ...
    onFocus() {
      if (localStorage.getItem("name")) {
        this.nameId = `${JSON.parse(localStorage.getItem("name")).nickname}.SearchHistory`;
      }else {
